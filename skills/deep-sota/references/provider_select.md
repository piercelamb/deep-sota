# Provider selection (read me after `validate-env.sh`)

You're here because `~/.config/lodestone/config.toml` is missing or
incomplete and `validate-env.sh` returned a JSON object. Inspect the
...
> exported in this shell. Set one and re-run `/deep-sota`.
...
If the validate-env JSON also includes a non-null `config_provider`
